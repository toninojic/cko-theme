...
if ( ! function_exists( 'cko_register_page_meta_boxes' ) ) {
	...
	function cko_register_page_meta_boxes() {
		add_meta_box(
			'cko-language-settings',
			__( 'CKO • Language Settings', 'cko-theme' ),
			'cko_render_language_settings_meta_box',
			'page',
			'side',
			'default'
		);
		...
		add_meta_box(
			'cko-frontpage-content',
			__( 'CKO • Front Page Content', 'cko-theme' ),
			'cko_render_frontpage_content_meta_box',
			'page',
			'normal',
			'default'
		);
		...
		add_meta_box(
			'cko-impact-items',
			__( 'CKO • Impact Items', 'cko-theme' ),
			'cko_render_impact_items_meta_box',
			'page',
			'normal',
			'default'
		);
...
if ( ! function_exists( 'cko_render_language_settings_meta_box' ) ) {
	...
	function cko_render_language_settings_meta_box( $post ) {
		...
		<p style="margin:0;color:#555;">
			<?php esc_html_e( 'Manually connect this page with the opposite language version. Language toggle will use this relation.', 'cko-theme' ); ?>
		</p>
		<?php
	}
}
...
if ( ! function_exists( 'cko_render_impact_items_meta_box' ) ) {
	/**
	 * Render repeatable impact items meta fields.
	 *
	 * @param WP_Post $post Current post object.
	 */
	function cko_render_impact_items_meta_box( $post ) {
		...
		?>
		<p style="margin-top:0;"><?php esc_html_e( 'Dodajte stavke za Impact grid. Svaka stavka ima naslov, broj/statistiku, opis i opcioni URL ikone/slike.', 'cko-theme' ); ?></p>
		<div id="cko-impact-items-wrap">
			<?php foreach ( $items as $index => $item ) : ?>
				<div class="cko-impact-row" style="border:1px solid #ddd;padding:12px;margin-bottom:12px;">
					...
					<p><label><?php esc_html_e( 'Broj / highlight', 'cko-theme' ); ?><input type="text" class="widefat" name="cko_impact_items[<?php echo esc_attr( $index ); ?>][value]" value="<?php echo esc_attr( isset( $item['value'] ) ? (string) $item['value'] : '' ); ?>"></label></p>
	...
	function cko_save_page_meta_fields( $post_id ) {
		...
		if ( isset( $_POST['cko_impact_items'] ) && is_array( $_POST['cko_impact_items'] ) ) {
			...
			foreach ( $raw_items as $item ) {
				...
				if ( '' === $clean_item['title'] && '' === $clean_item['value'] && '' === $clean_item['description'] && '' === $clean_item['icon'] ) {
					continue;
				}
			...
			if ( empty( $sanitized_items ) ) {
				delete_post_meta( $post_id, 'cko_impact_items' );
			} else {
				update_post_meta( $post_id, 'cko_impact_items', $sanitized_items );
			}
		}
	}
}
